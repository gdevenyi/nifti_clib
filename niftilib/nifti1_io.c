...
/* extension routines */
static int
nifti_read_extensions(nifti_image * nim, znzFile fp, int remain);
static int
...
static void
update_nifti_image_for_brick_list(nifti_image * nim, int nbricks);
static int
...
/* NBL routines */
static int
nifti_load_NBL_bricks(nifti_image * nim, const int * slist, const int * sindex, nifti_brick_list * NBL, znzFile fp);
static int
nifti_alloc_NBL_mem(const nifti_image * nim, int nbricks, nifti_brick_list * nbl);
static int
nifti_copynsort(int nbricks, const int * blist, int ** slist, int ** sindex);
static int
...
rci_read_data(nifti_image * nim,
              ...
              size_t        base_offset);
static int
rci_alloc_mem(void ** data, const int prods[8], int nprods, int nbyper);
static int
make_pivot_list(nifti_image * nim, const int dims[8], int pivots[8], int prods[8], int * nprods);
...
*//*----------------------------------------------------------------------*/
nifti_image *
nifti_image_read_bricks(const char * hname, int nbricks, const int * blist, nifti_brick_list * NBL)
{
  nifti_image * nim;
...
*//*--------------------------------------------------------------------*/
int
nifti_image_load_bricks(nifti_image * nim, int nbricks, const int * blist, nifti_brick_list * NBL)
{
...
*//*--------------------------------------------------------------------*/
int
valid_nifti_brick_list(nifti_image * nim, int nbricks, const int * blist, int disp_error)
{
  int c, nsubs;
  ...
  for (c = 0; c < nbricks; c++)
    ...
    {
      if (disp_error || g_opts.debug > 1)
        fprintf(stderr, "** volume index %d (#%d) is out of range [0,%d]\n", blist[c], c, nsubs - 1);
      return 0;
    }
...
*//*--------------------------------------------------------------------*/
int
nifti_disp_matrix_orient(const char * mesg, mat44 mat)
{
  int i, j, k;
...
nifti_mat44_inverse(mat44 R)
{
  ...
  deti = r11 * r22 * r33 - r11 * r32 * r23 - r21 * r12 * r33 + r21 * r32 * r13 + r31 * r12 * r23 -
...
*//*--------------------------------------------------------------------*/
void
nifti_swap_2bytes(size_t n, void * ar) /* 2 bytes at a time */
{
  size_t         ii;
...
*//*--------------------------------------------------------------------*/
void
nifti_swap_4bytes(size_t n, void * ar) /* 4 bytes at a time */
{
  size_t         ii;
...
*//*--------------------------------------------------------------------*/
void
nifti_swap_8bytes(size_t n, void * ar) /* 8 bytes at a time */
{
  size_t         ii;
  unsigned char *cp0 = (unsigned char *)ar, *cp1, *cp2;
  unsigned char  tval;
...
*//*--------------------------------------------------------------------*/
void
nifti_swap_16bytes(size_t n, void * ar) /* 16 bytes at a time */
{
  size_t         ii;
  unsigned char *cp0 = (unsigned char *)ar, *cp1, *cp2;
  unsigned char  tval;
...
*//*--------------------------------------------------------------------*/
void
nifti_swap_Nbytes(size_t n, int size, void * ar) /* subsuming case */
{
  switch (size)
  {
    case 2:
...
nifti_set_type_from_names(nifti_image * nim)
{
  ...
  if (!nim->fname || !nim->iname)
  {
    fprintf(stderr,
            ...
            (const void *)nim->fname,
            (const void *)nim->iname);
    return -1;
  }
...
*//*--------------------------------------------------------------------*/
nifti_image *
nifti_convert_nhdr2nim(struct nifti_1_header nhdr, const char * fname)
{
  int           ii, doswap, ioff;
...
*//*--------------------------------------------------------------------*/
nifti_1_header *
nifti_read_header(const char * hname, int * swapped, int check)
{
...
*//*--------------------------------------------------------------------*/
int
nifti_hdr_looks_good(const nifti_1_header * hdr)
{
  ...
  { /* ANALYZE 7.5 */
    ...
    if (g_opts.debug > 1) /* maybe tell user it's an ANALYZE hdr */
      fprintf(stderr, "-- nhdr magic field implies ANALYZE: magic = '%.4s'\n", hdr->magic);
...
*//*--------------------------------------------------------------------*/
nifti_image *
nifti_read_ascii_image(znzFile fp, char * fname, int flen, int read_data)
{
  nifti_image * nim;
  ...
  if (nim == NULL || nim->iname == NULL || nim->nbyper <= 0 || nim->nvox <= 0)
  {
    if (g_opts.debug > 0)
    {
      if (!nim)
        ...
        fprintf(stderr,
                "** ERROR: N_image_load: bad params (%p,%d,%u)\n",
                (const void *)nim->iname,
                nim->nbyper,
                (unsigned)nim->nvox);
    }
...
*//*--------------------------------------------------------------------*/
size_t
nifti_read_buffer(znzFile fp, void * dataptr, size_t ntot, nifti_image * nim)
{
  size_t ii;
...
*/
size_t
nifti_write_buffer(znzFile fp, const void * buffer, size_t numbytes)
{
...
*//*--------------------------------------------------------------------*/
nifti_1_header *
nifti_make_new_header(const int arg_dims[8], int arg_dtype)
{
  nifti_1_header * nhdr;
...
*//*--------------------------------------------------------------------*/
nifti_image *
nifti_make_new_nim(const int dims[8], int datatype, int data_fill)
{
  nifti_image *    nim;
  nifti_1_header * nhdr;
...
nifti_convert_nim2nhdr(const nifti_image * nim)
{
  struct nifti_1_header nhdr;
...
nifti_image_from_ascii(const char * str, int * bytes_read)
{
  ...
  if (nim->qform_code > 0)
    ...
    nim->qto_xyz = nifti_quatern_to_mat44(0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, nim->dx, nim->dy, nim->dz, 0.0F);
...
*//*-------------------------------------------------------------------------*/
int
nifti_read_collapsed_image(nifti_image * nim, const int dims[8], void ** data)
{
  znzFile fp;
...
*//*-------------------------------------------------------------------------*/
int
nifti_read_subregion_image(nifti_image * nim, const int * start_index, const int * region_size, void ** data)
{
