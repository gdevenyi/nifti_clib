...
main(int argc, const char * argv[])
{
  ...
  if (opts.disp_exts && ((rv = act_disp_exts(&opts)) != 0))
    FREE_RETURN(rv);
  if (opts.disp_hdr && ((rv = act_disp_hdrs(&opts)) != 0))
    FREE_RETURN(rv);
...
process_opts(int argc, const char * argv[], nt_opts * opts)
{
  ...
  for (ac = 1; ac < argc; ac++)
  {
    ...
    else if (!strncmp(argv[ac], "-add_comment_ext", 9))
    {
      ...
      opts->check_nim = 1;
    else if (!strncmp(argv[ac], "-copy_brick_list", 11) || !strncmp(argv[ac], "-copy_im", 10) ||
             !strncmp(argv[ac], "-cbl", 4))
    {
      opts->cbl = 1;
    }
    else if (!strncmp(argv[ac], "-copy_collapsed_image", 10) || !strncmp(argv[ac], "-cci", 4))
    {
    ...
    else if (!strncmp(argv[ac], "-disp_collapsed_image", 10) || !strncmp(argv[ac], "-disp_ci", 8))
    {
      /* we need to read in the 7 dimension values */
      int index;
      opts->ci_dims[0] = 0;
    ...
    else if (!strncmp(argv[ac], "-disp_ts", 10) || !strncmp(argv[ac], "-dts", 4))
    {
...
verify_opts(nt_opts * opts, const char * prog)
{
  ...
  ac += (opts->disp_hdr || opts->disp_nim || opts->disp_ana || opts->disp_exts) ? 1 : 0;
  ...
  else if ((opts->elist.len > 0 || opts->mod_hdr || opts->mod_nim || opts->swap_hdr || opts->swap_ana ||
            opts->swap_old) &&
           !opts->overwrite)
  {
...
act_disp_anas(nt_opts * opts)
{
  ...
  {
    /* do not validate the header structure */
    nhdr = (nifti_analyze75 *)nt_read_header(opts, opts->infiles.list[filenum], NULL, 0);
    if (!nhdr)
      return 1; /* errors are printed from library */
    ...
    if (g_debug > 1)
      fprintf(stderr, "-d analyze header is: %s\n", nifti_hdr_looks_good((nifti_1_header *)nhdr) ? "valid" : "invalid");
 ...
 *----------------------------------------------------------------------*/
int
fill_hdr_field_array(field_s * nh_fields)
{
  ...
  if (check_total_size("nifti_1_header test: ", nh_fields, NT_HDR_NUM_FIELDS, sizeof(nifti_1_header)))
    return 1;
 ...
 *----------------------------------------------------------------------*/
int
fill_nim_field_array(field_s * nim_fields)
{
  ...
  if (g_debug > 3) /* failure here is not an error condition */
    check_total_size("nifti_image test: ", nim_fields, NT_NIM_NUM_FIELDS, sizeof(nifti_image));
...
disp_nifti1_extension(const char * mesg, const nifti1_extension * ext, int maxlen)
{
  ...
  fprintf(stdout, "ecode = %d, esize = %d, edata = ", ext->ecode, ext->esize);

  if (!ext->edata)
    fprintf(stdout, "(NULL)\n");
  else if (ext->ecode == NIFTI_ECODE_AFNI || ext->ecode == NIFTI_ECODE_COMMENT)
  {
    len = ext->esize - 8;
 ...
 *----------------------------------------------------------------------*/
field_s *
get_hdr_field(const char * fname, int show_fail)
{
  field_s * fp;
  int       c;
 ...
 *----------------------------------------------------------------------*/
int
diff_hdrs(nifti_1_header * s0, nifti_1_header * s1, int display)
{
 ...
 *----------------------------------------------------------------------*/
int
diff_hdrs_list(nifti_1_header * s0, nifti_1_header * s1, str_list * slist, int display)
{
  field_s *     fp;
  const char ** sptr;
  int           c, ndiff = 0;
...
act_disp_ci(nt_opts * opts)
{
  ...
  {
    fprintf(stderr,
            "-d displaying time series at (i,j,k) = (%d,%d,%d)\n"
  ...
  for (filenum = 0; filenum < opts->infiles.len; filenum++)
  {
    ...
    if (g_debug > 0)
    {
      fprintf(stdout, "\ndataset '%s' @ (", nim->fname);
      if (opts->dts)
        disp_raw_data(opts->ci_dims + 1, DT_INT32, 3, ' ', 0);
      else
        disp_raw_data(opts->ci_dims + 1, DT_INT32, 7, ' ', 0);
      fprintf(stdout, ")\n");
    }
 ...
 *----------------------------------------------------------------------*/
nifti_image *
nt_image_read(nt_opts * opts, const char * fname, int doread)
{
  if (!opts || !fname)
  {
 ...
 *----------------------------------------------------------------------*/
nifti_1_header *
nt_read_header(nt_opts * opts, const char * fname, int * swapped, int check)
{
 ...
 *----------------------------------------------------------------------*/
nifti_image *
nt_read_bricks(nt_opts * opts, const char * fname, int len, int * list, nifti_brick_list * NBL)
{
  nifti_image * nim;
  int           c;
