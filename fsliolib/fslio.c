...
*/
const char *
FslFileTypeString(int filetype)
{
  if (filetype == FSL_TYPE_ANALYZE)
    return "ANALYZE-7.5";
  if (filetype == FSL_TYPE_NIFTI)
    return "NIFTI-1+";
  if (filetype == FSL_TYPE_NIFTI_PAIR)
    return "NIFTI-1";
  if (filetype == FSL_TYPE_ANALYZE_GZ)
    return "ANALYZE-7.5";
  if (filetype == FSL_TYPE_NIFTI_GZ)
    return "NIFTI-1+";
  if (filetype == FSL_TYPE_NIFTI_PAIR_GZ)
    return "NIFTI-1";
  return "UNKNOWN";
}
...
FslIsValidFileType(int filetype)
{
  if ((filetype != FSL_TYPE_ANALYZE) && (filetype != FSL_TYPE_ANALYZE_GZ) && (filetype != FSL_TYPE_NIFTI) &&
      (filetype != FSL_TYPE_NIFTI_GZ) && (filetype != FSL_TYPE_NIFTI_PAIR) && (filetype != FSL_TYPE_NIFTI_PAIR_GZ) &&
...
{
  if (fslio == NULL)
    FSLIOERR("FslGetFileType: Null pointer passed for FSLIO");
  if ((fslio->file_mode == FSL_TYPE_MINC) || (fslio->file_mode == FSL_TYPE_MINC_GZ))
  {
    return fslio->file_mode;
  }
  if (!FslIsValidFileType(fslio->file_mode))
    return -1;

  if (fslio->niftiptr != NULL)
  { /* check that it is nifti_type and filetype are consistent */
    if (fslio->niftiptr->nifti_type != FslBaseFileType(fslio->file_mode))
    {
      ...
      /* `const FSLIO *` makes the niftiptr member const, not what it points
         at, so the nifti_image may be updated without casting anything. */
...
int
FslGetFileType(const FSLIO * fslio)
{
...
void
FslSetFileType(FSLIO * fslio, int filetype)
{
  if (fslio == NULL)
    FSLIOERR("FslSetFileType: Null pointer passed for FSLIO");
  if ((filetype == FSL_TYPE_MINC) || (filetype == FSL_TYPE_MINC_GZ))
  {
    fslio->file_mode = filetype;
    return;
  }
  if (!FslIsValidFileType(filetype))
  {
    return;
  }
...
int
FslIsSingleFileType(int filetype)
{
  if ((filetype == FSL_TYPE_NIFTI) || (filetype == FSL_TYPE_NIFTI_GZ) || (filetype == FSL_TYPE_MINC) ||
      (filetype == FSL_TYPE_MINC_GZ))
    return 1;
  return 0;
}
...
int
FslIsCompressedFileType(int filetype)
{
  if (filetype >= 100)
    return 1;
  return 0;
...
{
  if (fslio == NULL)
    FSLIOERR("FslGetWriteMode: Null pointer passed for FSLIO");
  return fslio->write_mode;
}


void
FslSetWriteMode(FSLIO * fslio, int mode)
{
  if (fslio == NULL)
    FSLIOERR("FslSetWriteMode: Null pointer passed for FSLIO");
  fslio->write_mode = mode;
}


int
FslGetEnvOutputType(void)
{
  /* return type is one of FSL_TYPE_* or -1 to indicate error */
  char * otype;
  if (FslOverrideOutputType >= 0)
  ...
  if (strcmp(otype, "ANALYZE") == 0)
  {
    return FSL_TYPE_ANALYZE;
  }
  if (strcmp(otype, "ANALYZE_GZ") == 0)
  {
    return FSL_TYPE_ANALYZE_GZ;
  }
  if (strcmp(otype, "NIFTI") == 0)
  {
    return FSL_TYPE_NIFTI;
  }
  if (strcmp(otype, "NIFTI_GZ") == 0)
  {
    return FSL_TYPE_NIFTI_GZ;
  }
  if (strcmp(otype, "NIFTI_PAIR") == 0)
  {
    return FSL_TYPE_NIFTI_PAIR;
  }
  if (strcmp(otype, "NIFTI_PAIR_GZ") == 0)
  {
    return FSL_TYPE_NIFTI_PAIR_GZ;
  }
  if (strcmp(otype, "MINC") == 0)
  {
    return FSL_TYPE_MINC;
  }
  if (strcmp(otype, "MINC_GZ") == 0)
  {
    return FSL_TYPE_MINC_GZ;
  }
...
FslFileType(const char * fname)
{
  ...
  if (flen < 5)
    return retval; /* smallest name + extension is a.nii */
  if (strcmp(fname + flen - 4, ".nii") == 0)
    retval = FSL_TYPE_NIFTI;
  if (strcmp(fname + flen - 4, ".mnc") == 0)
    retval = FSL_TYPE_MINC;
  if (strcmp(fname + flen - 4, ".hdr") == 0)
    retval = FSL_TYPE_NIFTI_PAIR;
  if (strcmp(fname + flen - 4, ".img") == 0)
    retval = FSL_TYPE_NIFTI_PAIR;
  if ((retval == -1) && (flen < 8))
    return retval; /* small name + ext.gz is a.nii.gz */
  if (strcmp(fname + flen - 7, ".nii.gz") == 0)
    retval = FSL_TYPE_NIFTI_GZ;
  if (strcmp(fname + flen - 7, ".mnc.gz") == 0)
    retval = FSL_TYPE_MINC;
  if (strcmp(fname + flen - 7, ".hdr.gz") == 0)
    retval = FSL_TYPE_NIFTI_PAIR_GZ;
  if (strcmp(fname + flen - 7, ".img.gz") == 0)
    retval = FSL_TYPE_NIFTI_PAIR_GZ;
...
FslGetReadFileType(const FSLIO * fslio)
{
  ...
  if (fslio == NULL)
    FSLIOERR("FslReadGetFileType: Null pointer passed for FSLIO");
...
FslFileExists(const char * filename)
{
  ...
  if (hdrname != NULL)
  {
    imgname = nifti_findimgname(filename, FslBaseFileType(FslFileType(hdrname)));
    free(hdrname);
    if (imgname != NULL)
    {
      free(imgname);
...
char *
FslMakeBaseName(const char * fname)
{
  char * basename;
  int    blen;
...
FslInit4Write(FSLIO * fslio, const char * filename, int ft)
{
  ...
  {
    FslInitHeader(fslio, NIFTI_TYPE_FLOAT32, 1, 1, 1, 3, 0.0, 0.0, 0.0, 0.0, 4);
              ...
              size_t  dim)
{
  ...
  if (fslio == NULL)
    FSLIOERR("FslInitHeader: Null pointer passed for FSLIO");
...
void
FslCloneHeader(FSLIO * dest, const FSLIO * src)
{
  ...
  if (dest == NULL)
    FSLIOERR("FslCloneHeader: Null pointer passed for FSLIO");
  if (src == NULL)
    FSLIOERR("FslCloneHeader: Null pointer passed for FSLIO");
...
int
FslCheckForMultipleFileNames(const char * filename)
{
  ...
  ambiguous = 1;
  if ((hdrcount == 1) && (imgcount == 1) && (singlecount == 0))
  {
    ambiguous = 0;
  }
  if ((hdrcount == 0) && (imgcount == 0) && (singlecount == 1))
  {
    ambiguous = 0;
  }

  /* treat no image found as not ambiguous - want opening errors instead */
  if ((hdrcount == 0) && (imgcount == 0) && (singlecount == 0))
  {
    ambiguous = 0;
  }
 ...
 */
FSLIO *
FslOpen(const char * filename, const char * opts)
{
...
FslXOpen(const char * filename, const char * opts, int filetype)
{
  ...
  if (FslBaseFileType(FslGetFileType(fslio)) == FSL_TYPE_ANALYZE)
  {
    ...
    FslSetAnalyzeSform(fslio, orig, fslio->niftiptr->pixdim[1], fslio->niftiptr->pixdim[2], fslio->niftiptr->pixdim[3]);
  }
 ...
 */
void *
FslReadAllVolumes(FSLIO * fslio, char * filename)
{

  int imgtype;
  if (fslio == NULL)
    FSLIOERR("FslReadAllVolumes: Null pointer passed for FSLIO");
...
FslReadVolumes(FSLIO * fslio, void * buffer, size_t nvols)
{
  ...
  if (znz_isnull(fslio->fileptr))
    FSLIOERR("FslReadVolumes: Null file pointer");
  if (fslio->niftiptr != NULL)
  {
 ...
 */
void
FslWriteAllVolumes(FSLIO * fslio, const void * buffer)
{
  short x, y, z, t;

  if (fslio == NULL || fslio->niftiptr == NULL)
    FSLIOERR("FslWriteAllVolumes: Null pointer passed for FSLIO");
 ...
 */
size_t
FslWriteVolumes(FSLIO * fslio, const void * buffer, size_t nvols)
{
  /* The dimensions and datatype must be set before calling this function */
  size_t retval = 0; /* initialize to failure */
  if (fslio == NULL)
    FSLIOERR("FslWriteVolumes: Null pointer passed for FSLIO");
...
FslWriteHeader(FSLIO * fslio)
{
  ...
  if (fslio == NULL)
    FSLIOERR("FslWriteHeader: Null pointer passed for FSLIO");

  if (fslio->niftiptr != NULL)
  {
    fslio->written_hdr = 1;
    if (znz_isnull(fslio->fileptr))
      FSLIOERR("FslWriteHeader: no file opened!");
 ...
 */
size_t
FslReadSliceSeries(FSLIO * fslio, void * buffer, short slice, size_t nvols)
{
  ...
  if (fslio == NULL)
    FSLIOERR("FslReadSliceSeries: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
    ...
    if ((slice < 0) || (slice >= z))
      FSLIOERR("FslReadSliceSeries: slice outside valid range");

    slbytes = (size_t)x * (size_t)y * (size_t)(FslGetDataType(fslio, &type) / 8);
    ...
    {
      if (n > 0)
        znzseek(fslio->fileptr, (znz_off_t)(volbytes - slbytes), SEEK_CUR);
      if (znzread((char *)buffer + n * slbytes, 1, slbytes, fslio->fileptr) != slbytes)
        FSLIOERR("FslReadSliceSeries: failed to read values");
      /* swapsize is 0 for the 8-bit and RGB datatypes, which need no
         swapping; dividing by it would be a division by zero. */
      if (fslio->niftiptr->byteorder != nifti_short_order() && fslio->niftiptr->swapsize > 1)
        nifti_swap_Nbytes(
          slbytes / (size_t)fslio->niftiptr->swapsize, fslio->niftiptr->swapsize, (char *)buffer + n * slbytes);
    }
 ...
 */
size_t
FslReadRowSeries(FSLIO * fslio, void * buffer, short row, short slice, size_t nvols)
{
  ...
  if (fslio == NULL)
    FSLIOERR("FslReadRowSeries: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
    ...
    if ((slice < 0) || (slice >= z))
      FSLIOERR("FslReadRowSeries: slice outside valid range");
    if ((row < 0) || (row >= y))
      FSLIOERR("FslReadRowSeries: row outside valid range");
    ...
    {
      if (n > 0)
        znzseek(fslio->fileptr, (znz_off_t)(volbytes - rowbytes), SEEK_CUR);
      if (znzread((char *)buffer + n * rowbytes, 1, rowbytes, fslio->fileptr) != rowbytes)
        FSLIOERR("FslReadRowSeries: failed to read values");
      /* see the note in FslReadSliceSeries about swapsize == 0 */
      if (fslio->niftiptr->byteorder != nifti_short_order() && fslio->niftiptr->swapsize > 1)
        nifti_swap_Nbytes(
          rowbytes / (size_t)fslio->niftiptr->swapsize, fslio->niftiptr->swapsize, (char *)buffer + n * rowbytes);
    }
 ...
 */
size_t
FslReadTimeSeries(FSLIO * fslio, void * buffer, short xVox, short yVox, short zVox, size_t nvols)
{
  ...
  if (fslio == NULL)
    FSLIOERR("FslReadTimeSeries: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
    ...
    if ((xVox < 0) || (xVox >= xdim))
      FSLIOERR("FslReadTimeSeries: voxel outside valid range");
    if ((yVox < 0) || (yVox >= ydim))
      FSLIOERR("FslReadTimeSeries: voxel outside valid range");
    if ((zVox < 0) || (zVox >= zdim))
      FSLIOERR("FslReadTimeSeries: voxel outside valid range");
    ...
    {
      if (n > 0)
        znzseek(fslio->fileptr, (znz_off_t)(volbytes - wordsize), SEEK_CUR);
      if (znzread((char *)buffer + (n * wordsize), 1, wordsize, fslio->fileptr) != wordsize)
        FSLIOERR("FslReadTimeSeries: failed to read values");
      if (fslio->niftiptr->byteorder != nifti_short_order())
        nifti_swap_Nbytes(1, fslio->niftiptr->swapsize, (char *)buffer + (n * wordsize));
    }
...
size_t
FslGetVolSize(FSLIO * fslio)
{
  /* returns number of voxels per 3D volume */
  if (fslio == NULL)
    FSLIOERR("FslGetVolSize: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
    return (size_t)fslio->niftiptr->nx * (size_t)fslio->niftiptr->ny * (size_t)fslio->niftiptr->nz;
  }
  if (fslio->mincptr != NULL)
  {
    fprintf(stderr, "Warning:: Minc is not yet supported\n");
...
void
FslSetDim(FSLIO * fslio, short x, short y, short z, short v)
{
  int ndim;
  if (fslio == NULL)
    FSLIOERR("FslSetDim: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
    ...
    fslio->niftiptr->nvox = (size_t)fslio->niftiptr->nx * (size_t)fslio->niftiptr->ny * (size_t)fslio->niftiptr->nz *
                            (size_t)fslio->niftiptr->nt * (size_t)fslio->niftiptr->nu * (size_t)fslio->niftiptr->nv *
...
void
FslGetDim(FSLIO * fslio, short * x, short * y, short * z, short * v)
{
  if (fslio == NULL)
    FSLIOERR("FslGetDim: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
    *x = fslio->niftiptr->nx;
    *y = fslio->niftiptr->ny;
...
void
FslGetDimensionality(FSLIO * fslio, size_t * dim)
{
  if (fslio == NULL)
    FSLIOERR("FslGetDimensionality: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
void
FslSetVoxDim(FSLIO * fslio, float x, float y, float z, float tr)
{
  if (fslio == NULL)
    FSLIOERR("FslSetVoxDim: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
void
FslGetVoxDim(FSLIO * fslio, float * x, float * y, float * z, float * tr)
{
  if (fslio == NULL)
    FSLIOERR("FslGetVoxDim: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
void
FslGetCalMinMax(FSLIO * fslio, float * min, float * max)
{
  if (fslio == NULL)
    FSLIOERR("FslGetCalMinMax: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
void
FslSetCalMinMax(FSLIO * fslio, float min, float max)
{
  if (fslio == NULL)
    FSLIOERR("FslSetCalMinMax: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
void
FslGetAuxFile(FSLIO * fslio, char * aux_file)
{
  if (fslio == NULL)
    FSLIOERR("FslGetAuxFile: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
void
FslSetAuxFile(FSLIO * fslio, const char * aux_file)
{
  if (fslio == NULL)
    FSLIOERR("FslSetAuxFile: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
FslSetVoxUnits(FSLIO * fslio, const char * units)
{
  int unitcode = 0;
  if (fslio == NULL)
    FSLIOERR("FslSetVoxUnits: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
void
FslSetTimeUnits(FSLIO * fslio, const char * units)
{
  int unitcode = 0;
  if (fslio == NULL)
    FSLIOERR("FslSetTimeUnits: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
void
FslGetTimeUnits(FSLIO * fslio, char * units)
{
  if (fslio == NULL)
    FSLIOERR("FslGetTimeUnits: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
void
FslSetDataType(FSLIO * fslio, short t)
{
  int nbytepix = 0, ss = 0;
  if (fslio == NULL)
    FSLIOERR("FslSetDataType: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
size_t
FslGetDataType(FSLIO * fslio, short * t)
{
  /* returns bits per pixel */
  int nbytepix = 32, ss = 0;
  if (fslio == NULL)
    FSLIOERR("FslGetDataType: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
void
FslGetMMCoord(mat44 stdmat, float voxx, float voxy, float voxz, float * mmx, float * mmy, float * mmz)
{
  *mmx = stdmat.m[0][0] * voxx + stdmat.m[0][1] * voxy + stdmat.m[0][2] * voxz + stdmat.m[0][3];
  *mmy = stdmat.m[1][0] * voxx + stdmat.m[1][1] * voxy + stdmat.m[1][2] * voxz + stdmat.m[1][3];
  *mmz = stdmat.m[2][0] * voxx + stdmat.m[2][1] * voxy + stdmat.m[2][2] * voxz + stdmat.m[2][3];
}


void
FslGetVoxCoord(mat44 stdmat, float mmx, float mmy, float mmz, float * voxx, float * voxy, float * voxz)
{
  mat44 mm2vox;

  mm2vox = nifti_mat44_inverse(stdmat);
  *voxx = mm2vox.m[0][0] * mmx + mm2vox.m[0][1] * mmy + mm2vox.m[0][2] * mmz + mm2vox.m[0][3];
  *voxy = mm2vox.m[1][0] * mmx + mm2vox.m[1][1] * mmy + mm2vox.m[1][2] * mmz + mm2vox.m[1][3];
  *voxz = mm2vox.m[2][0] * mmx + mm2vox.m[2][1] * mmy + mm2vox.m[2][2] * mmz + mm2vox.m[2][3];
}


void
FslSetStdXform(FSLIO * fslio, short sform_code, mat44 stdmat)
{
  /* NB: stdmat must point to a 4x4 array */
  if (fslio == NULL)
    FSLIOERR("FslSetStdXform: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
short
FslGetStdXform(FSLIO * fslio, mat44 * stdmat)
{
  /* returns sform code  (NB: stdmat must point to a 4x4 array) */
  float dx, dy, dz, tr;
  if (fslio == NULL)
    FSLIOERR("FslGetStdXform: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
void
FslSetRigidXform(FSLIO * fslio, short qform_code, mat44 rigidmat)
{
  /* NB: rigidmat must point to an allocated mat44 */
  float dx, dy, dz;
  if (fslio == NULL)
    FSLIOERR("FslSetRigidXform: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
    ...
    nifti_mat44_to_quatern(fslio->niftiptr->qto_xyz,
                           &(fslio->niftiptr->quatern_b),
                           &(fslio->niftiptr->quatern_c),
                           &(fslio->niftiptr->quatern_d),
                           &(fslio->niftiptr->qoffset_x),
                           &(fslio->niftiptr->qoffset_y),
                           &(fslio->niftiptr->qoffset_z),
                           &dx,
                           &dy,
                           &dz,
                           &(fslio->niftiptr->qfac));
...
short
FslGetRigidXform(FSLIO * fslio, mat44 * rigidmat)
{
  /* returns qform code  (NB: rigidmat must point to an allocated mat44) */
  float dx, dy, dz, tr;
  if (fslio == NULL)
    FSLIOERR("FslGetRigidXform: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
void
FslSetIntent(FSLIO * fslio, short intent_code, float p1, float p2, float p3)
{
  if (fslio == NULL)
    FSLIOERR("FslSetIntent: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
short
FslGetIntent(FSLIO * fslio, short * intent_code, float * p1, float * p2, float * p3)
{
  /* also returns intent code */
  if (fslio == NULL)
    FSLIOERR("FslGetIntent: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
{
  if (fslio == NULL)
    FSLIOERR("FslSetIntensityScaling: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
int
FslGetIntensityScaling(FSLIO * fslio, float * slope, float * intercept)
{
  /* returns 1 if scaling required or 0 otherwise */
  if (fslio == NULL)
    FSLIOERR("FslGetIntensityScaling: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
int
FslGetLeftRightOrder(FSLIO * fslio)
{
  ...
  if (fslio == NULL)
    FSLIOERR("FslGetLeftRightOrder: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
    ...
    if (det < 0.0)
      order = FSL_RADIOLOGICAL;
    else
      order = FSL_NEUROLOGICAL;
  }
  if (fslio->mincptr != NULL)
  {
    fprintf(stderr, "Warning:: Minc is not yet supported\n");
...
void
FslSetAnalyzeSform(FSLIO * fslio, const short * orig, float dx, float dy, float dz)
{
  ...
  float x, y, z;
  if (fslio == NULL)
    FSLIOERR("FslSetAnalyzeSform: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
    if (FslBaseFileType(FslGetFileType(fslio)) == FSL_TYPE_ANALYZE)
    {
    ...
    if ((orig[0] != 0) || (orig[1] != 0) || (orig[2] != 0))
    {
      ...
      else
      {
        ...
        x = fslio->niftiptr->sto_xyz.m[0][0] * origx + fslio->niftiptr->sto_xyz.m[0][1] * origy +
            fslio->niftiptr->sto_xyz.m[0][2] * origz + fslio->niftiptr->sto_xyz.m[0][3];
        y = fslio->niftiptr->sto_xyz.m[1][0] * origx + fslio->niftiptr->sto_xyz.m[1][1] * origy +
            fslio->niftiptr->sto_xyz.m[1][2] * origz + fslio->niftiptr->sto_xyz.m[1][3];
        z = fslio->niftiptr->sto_xyz.m[2][0] * origx + fslio->niftiptr->sto_xyz.m[2][1] * origy +
            fslio->niftiptr->sto_xyz.m[2][2] * origz + fslio->niftiptr->sto_xyz.m[2][3];
...
void
FslGetAnalyzeOrigin(FSLIO * fslio, short orig[5])
{
  /* NB: orig returned here is in Analyze convention - starting at 1, not 0 */
  if (fslio == NULL)
    FSLIOERR("FslGetAnalyzeOrigin: Null pointer passed for FSLIO");
  if (fslio->niftiptr != NULL)
  {
...
FslClose(FSLIO * fslio)
{
  ...
  /* close the (data) file */
  if (!znz_isnull(fslio->fileptr))
    retval = znzclose(fslio->fileptr);

  /** ----- if writing the image, need to worry about the header bit ----- **/

  if ((fslio->niftiptr != NULL) && (FslGetWriteMode(fslio) == 1) && (fslio->written_hdr == 0))
  {
...
int
FslReadRawHeader(void * buffer, const char * filename)
{
  znzFile fp;
  int     retval;
...
void
FslSetOverrideOutputType(int type)
{
...
int
FslGetOverrideOutputType(void)
{
  return FslOverrideOutputType;
}

void
FslSetIgnoreMFQ(int flag)
{
 ...
 */
double ***
FslGetVolumeAsScaledDouble(FSLIO * fslio, int vol)
{
  double *** newbuf;
  void *     diskbuf;
  int        xx, yy, zz;
  ...
  if (fslio == NULL)
    FSLIOERR("FslGetVolumeAsScaledDouble: Null pointer passed for FSLIO");
  ...
  if (fslio->niftiptr != NULL)
  {
    ...
    ret =
      convertBufferToScaledDouble(newbuf[0][0], diskbuf, (long)(xx * yy * zz), slope, inter, fslio->niftiptr->datatype);
...
FslGetBufferAsScaledDouble(FSLIO * fslio)
{
  ...
  if (fslio == NULL)
    FSLIOERR("FslGetBufferAsScaledDouble: Null pointer passed for FSLIO");
  ...
  if (fslio->niftiptr != NULL)
  {
    ...
    /** cvt it */
    ret = convertBufferToScaledDouble(
      newbuf[0][0][0], fslio->niftiptr->data, (long)(xx * yy * zz * tt), slope, inter, fslio->niftiptr->datatype);
 ...
 */
int
convertBufferToScaledDouble(double * outbuf, void * inbuf, long len, float slope, float inter, int nifti_datatype)
{
...
d3matrix(int zh, int yh, int xh)
{
  ...
  if (!t)
    FSLIOERR("d3matrix: allocation failure");

  /** allocate pointers for ydim */
  t[0] = (double **)malloc((size_t)nslice * (size_t)nrow * sizeof(double *));
  if (!t[0])
    FSLIOERR("d3matrix: allocation failure");


  /** allocate the data blob */
  t[0][0] = (double *)malloc((size_t)nslice * (size_t)nrow * (size_t)ncol * sizeof(double));
  if (!t[0][0])
    FSLIOERR("d3matrix: allocation failure");


  /** point everything to the data blob */
  for (j = 1; j < nrow * nslice; j++)
    t[0][j] = t[0][j - 1] + ncol;
  for (j = 1; j < nslice; j++)
    t[j] = t[j - 1] + nrow;
...
d4matrix(int th, int zh, int yh, int xh)
{
  ...
  if (!t)
    FSLIOERR("d4matrix: allocation failure");
  ...
  /** point everything to the data blob */
  for (j = 1; j < nrow * nslice * nvol; j++)
    t[0][0][j] = t[0][0][j - 1] + ncol;
  for (j = 1; j < nslice * nvol; j++)
    t[0][j] = t[0][j - 1] + nrow;
  for (j = 1; j < nvol; j++)
    t[j] = t[j - 1] + nslice;
