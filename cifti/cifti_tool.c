...
int disp_cifti_extension(nifti_image * nim, opts_t * opts)
{
   ...
   /* find the CIFTI extension, if there is one.  This loop used to test
      ext_list[0] on every iteration without ever advancing, so it could
      only ever match an extension that happened to be first. */
   ext = NULL;
...
int ax_has_bdata(FILE * fp, afni_xml_t * ax, int depth)
{
   ...
   if( gopt.verb > 2 && ax->blen > 1 ) {
      if( ax->btype == NIFTI_TYPE_FLOAT64 ) {
         ...
         if(dp) { /* repetitive checks to appease compilers */
           fprintf(fp, " = %lf  %lf  ...\n", dp[0], dp[1]);
         }
