...
/* processing */
int
disp_cifti_extension(nifti_image * nim, opts_t * opts);
int
disp_hex_data(const char * mesg, const void * data, int len, FILE * fp);
int
eval_cifti_extension(afni_xml_t * ax, opts_t * opts);
int
show_cifti_summary(FILE * fp, afni_xml_t * ax, int verb);

/* main */
int
process_args(int argc, const char * argv[], opts_t * opts);
int
process(opts_t * opts);
int
show_help(void);
...
show_hist(void);
int
write_extension(FILE * fp, nifti1_extension * ext, int maxlen);

/* recur */
int
ax_has_data(FILE * fp, afni_xml_t * ax, int depth);
int
ax_has_bdata(FILE * fp, afni_xml_t * ax, int depth);
int
ax_num_tokens(FILE * fp, afni_xml_t * ax, int depth);
int
ax_show_text_data(FILE * fp, afni_xml_t * ax, int depth);
int
ax_show_names(FILE * fp, afni_xml_t * ax, int depth);

/* stream */
int
close_stream(FILE * fp);
FILE *
open_write_stream(const char * fname);
...
main(int argc, const char * argv[])
{
  ...
  if (rv < 0)
    return 1; /* error */
  if (rv > 0)
    return 0; /* non-error termination */
...
process_args(int argc, const char * argv[], opts_t * opts)
{
  ...
  for (ac = 1; ac < argc; ac++)
  {
    ...
    else if (!strcmp(argv[ac], "-as_cext") || !strcmp(argv[ac], "-as_cifti_ext"))
    {
      opts->as_cext = 1;
    }
...
process(opts_t * opts)
{
  ...
  if (opts->as_cext)
    ax = axio_read_file(opts->fin);
  else
    axio_read_cifti_file(opts->fin, 0, &nim, &ax);
  ...
  if (opts->disp_cext)
    disp_cifti_extension(nim, opts);
  if (opts->eval_cext)
    eval_cifti_extension(ax, opts);
...
eval_cifti_extension(afni_xml_t * ax, opts_t * opts)
{
  ...
  if (opts->verb > 1)
    fprintf(stderr, "-- recursive eval from %s\n", opts->eval_type ? opts->eval_type : "NULL");
...
ax_has_data(FILE * fp, afni_xml_t * ax, int depth)
{
  ...
  if (!ax->xtext && ax->xlen <= 0 && !ax->bdata && ax->blen <= 0)
    return 0;
...
ax_has_bdata(FILE * fp, afni_xml_t * ax, int depth)
{
  ...
  if (gopt.verb > 1)
    fprintf(fp, "%*sdata in depth %d ", depth * 3, "", depth);
  ...
  if (gopt.verb > 2 && ax->blen > 1)
  {
    if (ax->btype == NIFTI_TYPE_FLOAT64)
    {
      ...
      if (dp)
      { /* repetitive checks to appease compilers */
        fprintf(fp, " = %lf  %lf  ...\n", dp[0], dp[1]);
      }
    }
...
show_help(void)
{
  printf("ct : short example of reading/writing CIFTI-2 datasets\n"
         "\n"
