/* strdup() is POSIX, not ISO C11; ask for it explicitly.  Must precede
   every #include.  (It is in C23, so this can go once C23 is the floor.) */
...
int
axml_get_dstore(void)
{
  return gAXD.dstore;
}
int
axml_set_dstore(int val)
{
  if (val)
    gAXD.dstore = 1; /* includes -1 as applying the default */
  else
    gAXD.dstore = 0;
  return 0;
}
...
int
axml_get_indent(void)
{
  return gAXD.indent;
}
int
axml_set_indent(int val)
{
  if (val == -1)
    gAXD.indent = 3;
  else if (val >= 0)
...
int
axml_get_buf_size(void)
{
  return gAXD.buf_size;
}
int
axml_set_buf_size(int val)
{
  if (val == -1)
    gAXD.buf_size = AXML_DEF_BSIZE;
  else if (val > 0)
    gAXD.buf_size = val;
  else
    return 1; /* failure - no action */
...
FILE *
axml_get_wstream(void)
{
  return gAXD.wstream;
}
int
axml_set_wstream(FILE * fp)
{
  gAXD.wstream = fp;
  return 0;
}
...
static void XMLCALL
cb_char(void *, const char *, int);
static void XMLCALL
cb_default(void *, const char *, int);
static void XMLCALL
cb_start_ele(void *, const char *, const char **);
static void XMLCALL
cb_stop_ele(void *, const char *);

static int
epush(afni_xml_control *, const char *, const char **);
static int
epop(afni_xml_control *, const char *);

static int
add_to_xchild_list(afni_xml_t * parent, afni_xml_t * child);
static int
add_to_xroot_list(afni_xml_control * xd, afni_xml_t * newp);
static int
append_to_string(char **, int *, const char *, int);
static int
disp_axml_ctrl(const char * mesg, afni_xml_control * dp, int show_all);
static int
disp_gen_text(afni_xml_control *, const char *, const char *, int);
static void
free_whitespace(void);
static int
init_axml_ctrl(afni_xml_control * xd, int doall);
static int
process_popped_element(afni_xml_control * xd, const char * ename);
static int
reset_xml_buf(afni_xml_control * xd, char ** buf, int * bsize);
static int
white_first(const char * str, int len);
static int
white_last(const char * str, int len);

static int
show_depth(afni_xml_control *, int show);
static int
show_attrs(afni_xml_control *, const char **, int);
...
afni_xml_list
axml_read_file(const char * fname, int read_data)
{
  ...
  if (xd->verb > 1)
    fprintf(stderr, "++ done parsing XML file %s\n", fname);
...
afni_xml_list
axml_read_buf(const char * buf_in, int64_t bin_len)
{
  afni_xml_control * xd = &gAXD; /* point to global struct */
  afni_xml_list      xlist = { 0, NULL };
  ...
  if (xd->verb > 1)
    fprintf(stderr, "++ done parsing XML buffer\n");
...
int
axml_disp_xlist(const char * mesg, afni_xml_list * axlist, int verb)
{
  FILE * fp = stderr;
  int    ind;

  if (gAXD.wstream != NULL)
    fp = gAXD.wstream;
...
int
axml_disp_xml_t(const char * mesg, afni_xml_t * ax, int indent, int verb)
{
  FILE * fp = stderr;
  int    ind;
  ...
  if (verb > 1)
  {
    ...
    if (verb > 2)
    {
      if (ax->xtext)
      {
        if (verb > 3)
          ...
          fprintf(fp, "%*sxtext   : %.30s...\n", indent, "", ax->xtext);
      }
      else
        fprintf(fp, "%*sxtext  : NULL\n", indent, "");
      fprintf(fp, "%*sxlen   : %d\n", indent, "", ax->xlen);
      fprintf(fp, "%*scdata  : %d\n", indent, "", ax->cdata);
    ...
    if (verb > 3)
      fprintf(fp, "%*sxparent : %s\n", indent, "", ax->xparent ? "SET" : "NONE");
  }
  else
    /* just show the name */
    fprintf(fp, "%*s%s\n", indent, "", ax->name);
...
int
axml_add_attrs(afni_xml_t * ax, const char ** attr)
{
  int c, aind, natr;
...
int
axml_free_xlist(afni_xml_list * axlist)
{
  int ind;
...
axml_free_xml_t(afni_xml_t * ax)
{
  ...
  {
    if (ax->attrs.name)
      free(ax->attrs.name[ind]);
    if (ax->attrs.value)
      free(ax->attrs.value[ind]);
  }
  free(ax->attrs.name);
  ax->attrs.name = NULL;
  free(ax->attrs.value);
...
int
axml_recur(int (*func)(FILE * fp, afni_xml_t *, int), afni_xml_t * ax)
{
  static int depth = 1;
  int        ind;
...
char *
axml_attr_value(afni_xml_t * ax, const char * name)
{
  int ind;
  if (!ax)
    return NULL;
  for (ind = 0; ind < ax->attrs.length; ind++)
    if (!strcmp(ax->attrs.name[ind], name))
      return ax->attrs.value[ind];
...
static XML_Parser
init_xml_parser(void * user_data)
{
...
static void XMLCALL
cb_start_ele(void * udata, const char * ename, const char ** attr)
{
  afni_xml_control * xd = (afni_xml_control *)udata;

  if (xd->wkeep)
    xd->wkeep = 0; /* clear storage continuation */

  (void)epush(xd, ename, attr);
}

static void XMLCALL
cb_stop_ele(void * udata, const char * ename)
{
  afni_xml_control * xd = (afni_xml_control *)udata;
...
static int
epush(afni_xml_control * xd, const char * ename, const char ** attr)
{
...
static int
epop(afni_xml_control * xd, const char * ename)
{
  if (xd->wkeep)
    xd->wkeep = 0; /* clear storage continuation */
...
static int
show_attrs(afni_xml_control * xd, const char ** attr, int showd)
{
  int count;
...
static void XMLCALL
cb_char(void * udata, const char * cdata, int length)
{
  afni_xml_control * xd = (afni_xml_control *)udata;
  afni_xml_t *       parent;
  ...
  {
    if (xd->verb > 4)
      fprintf(stderr, "-- skipping white char [%d]\n", length);
    return;
  }
  ...
  if (!xd->wkeep)
    xd->wkeep = 1; /* store everything at this point */

  if (xd->verb > 4)
    disp_gen_text(xd, "char", cdata, length);
...
static int
append_to_string(char ** ostr, int * olen, const char * istr, int ilen)
{
  int newlen;
...
static int
disp_gen_text(afni_xml_control * xd, const char * header, const char * cdata, int length)
{
  ...
  show_depth(xd, 1);
  fprintf(stderr, "%s [len %d]: '%s'\n", header, length, strip_whitespace(str, len));

  return 0;
}

static void XMLCALL
cb_default(void * udata, const char * cdata, int length)
{
  afni_xml_control * xd = (afni_xml_control *)udata;
  ...
  if (xd->verb > 5)
    disp_gen_text(xd, "default XML", cdata, length);
}


static int
show_depth(afni_xml_control * xd, int show)
{
  FILE * fp = stderr;
  if (xd->wstream)
    fp = xd->wstream;
  ...
  if (show)
    fprintf(fp, "%*s %02d ", xd->indent * xd->depth, "", xd->depth);
  else
    fprintf(fp, "%*s    ", xd->indent * xd->depth, "");
  return 0;
}
