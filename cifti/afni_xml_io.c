...
/* local protos */
static afni_xml_t *
xlist_to_ax1(afni_xml_list * xlist);

static int
axio_alloc_known_data(FILE * fp, afni_xml_t * ax, int depth);
static int
can_process_dtype(int dtype);
static int
dalloc_as_nifti_type(FILE * fp, afni_xml_t * ax, int64_t nvals, int);

static void
disp_name_n_desc(FILE * fp, afni_xml_t * ax, int indent, int verb);
static void
disp_brainmodel_child(FILE * fp, afni_xml_t * ax, int verb);
static void
disp_namedmap_child(FILE * fp, afni_xml_t * ax, int verb);
static void
disp_parcel_child(FILE * fp, afni_xml_t * ax, int verb);
static void
disp_surface_child(FILE * fp, afni_xml_t * ax, int verb);
static void
disp_volume_child(FILE * fp, afni_xml_t * ax, int verb);

static int64_t
text_to_i64(int64_t * result, const char * text, int64_t nvals);
static int64_t
text_to_f64(double * result, const char * text, int64_t nvals);
 ...
 */
int
axio_read_cifti_file(const char * fname, int get_ndata, nifti_image ** nim_out, afni_xml_t ** ax_out)
{
...
afni_xml_t *
axio_read_buf(const char * buf, int64_t blen)
{
  afni_xml_list xlist;

  /* try to read file */
  xlist = axml_read_buf(buf, blen);
  if (xlist.len <= 0 || !xlist.xlist)
    return NULL;
...
/* read file into a single, allocated afni_xml_t struct */
afni_xml_t *
axio_read_file(const char * fname)
{
  afni_xml_list xlist;
...
int
axio_text_to_binary(afni_xml_t * ax)
{
  if (!ax)
    return 0;
...
int
axio_num_tokens(const char * str, int64_t maxlen)
{
  const char * sp = str;
...
afni_xml_t *
axio_cifti_from_ext(nifti_image * nim)
{
...
int
axio_show_attrs(FILE * fp, afni_xml_t * ax, int indent)
{
  ...
  for (ind = 0; ind < ax->attrs.length; ind++)
    fprintf(ofp, "%*s%-*s = %s\n", indent, "", maxl, ax->attrs.name[ind], ax->attrs.value[ind]);
  return 0;
}

/* assume single Matrix for now */
int
axio_show_cifti_summary(FILE * fp, char * mesg, afni_xml_t * ax, int verb)
{
  FILE *      ofp = fp ? fp : stderr;
  afni_xml_t *ac, *am;
  int         ind;
  ...
  if (verb > 1)
    fprintf(ofp, "-- have %d MIMap/MD elements\n", ac->nchild);

  for (ind = 0; ind < ac->nchild; ind++)
  {
    am = ac->xchild[ind];
    if (strcmp(am->name, "MatrixIndicesMap"))
      continue;
...
static gen_disp_func_t MIM_disp_funcs[AXIO_NMIM_KIDS] = { disp_namedmap_child,
                                                          disp_surface_child,
                                                          disp_parcel_child,
                                                          disp_volume_child,
                                                          disp_brainmodel_child };

static int
get_map_index(afni_xml_t * ax)
{
  int kid;
...
int
axio_show_mim_summary(FILE * fp, const char * mesg, afni_xml_t * ax, int verb)
{
  afni_xml_t *xm, *xt;
  FILE *      ofp = fp ? fp : stderr;
  int         kid, matkid, mind;
  ...
  if (verb > 1)
    fprintf(ofp, "-- have %d Matrix children\n", xm->nchild);

  for (matkid = 0; matkid < xm->nchild; matkid++)
  {
    xt = xm->xchild[matkid];
    if (strcmp(xt->name, "MatrixIndicesMap"))
      continue;

    if (verb > 1)
      fprintf(ofp, "-- have %d MIMap children\n", xt->nchild);

    for (kid = 0; kid < xt->nchild; kid++)
    {
      mind = get_map_index(xt->xchild[kid]);
      /* get_map_index() returns -1 for any element name it does not
         recognise; the test here used to be on kid, the loop counter,
         which is never negative. */
      if (mind >= 0)
        MIM_disp_funcs[mind](ofp, xt->xchild[kid], verb);
    }
  }
 ...
 */
afni_xml_t *
axio_find_map_name(afni_xml_t * ax, const char * name, int maxd)
{
  afni_xml_t * rv;
  int          ind;
  ...
  if (ax->name && !strcmp(ax->name, name))
    return ax;
...
disp_name_n_desc(FILE * fp, afni_xml_t * ax, int indent, int verb)
{
  ...
  if (ax->xtext && ax->xlen > 0)
  {
    ...
    if (verb > 1 && ax->blen > 0)
      fprintf(fp, "%*s: %" PRId64 " values of type %s\n", indent + 3, "", ax->blen, nifti_datatype_string(ax->btype));
  }
  else
    fputc('\n', fp);

  if (verb > 1)
    axio_show_attrs(fp, ax, indent + 6);
}
...
disp_namedmap_child(FILE * fp, afni_xml_t * ax, int verb)
{
  ...
  if (xl)
    fprintf(fp, "         with length %d LabelTable\n", xl->nchild);
...
axio_alloc_known_data(FILE * fp, afni_xml_t * ax, int depth)
{
  ...
  if (!strcmp(ax->name, "VertexIndices"))
  {
    ...
    if (!cp)
    {
      fprintf(fp, "** axAKD: no IndexCount\n");
      return 1;
    }
  ...
  if (!strcmp(ax->name, "VoxelIndicesIJK"))
  {
    ...
    if (!cp)
    {
      fprintf(fp, "** axAKD: no ijk IndexCount\n");
      return 1;
    }
...
static int
dalloc_as_nifti_type(FILE * fp, afni_xml_t * ax, int64_t nvals, int dtype)
{
  int     nbyper = 0;
  int64_t nread, ntok;
  ...
  if (nvals >= 0)
    ...
    ntok = axio_num_tokens(ax->xtext, ax->xlen);
...
can_process_dtype(int dtype)
{
  if (dtype == NIFTI_TYPE_INT64)
    return 1;
  if (dtype == NIFTI_TYPE_FLOAT64)
    return 1;
...
xlist_to_ax1(afni_xml_list * xlist)
{
  ...
  if (xlist->len > 1)
  {
    ...
    for (c = 0; c < xlist->len; c++)
      axml_free_xml_t(xlist->xlist[c]);
  }
