...
/* local prototypes */
static int
free_opts_mem(nt_opts * nopt);
static int
num_volumes(nifti_image * nim);
static char *
read_file_text(const char * filename, int * length);


#define NTL_FERR(func, msg, file) fprintf(stderr, "** ERROR (%s): %s '%s'\n", func, msg, file)
...
static const char * g_hdr_timing_fnames[NT_HDR_TIME_NFIELDS] = { "slice_code",     "slice_start", "slice_end",
                                                                 "slice_duration", "dim_info",    "dim",
                                                                 "pixdim",         "xyzt_units" };
static const char * g_nim_timing_fnames[NT_NIM_TIME_NFIELDS] = { "slice_code",     "slice_start", "slice_end",
                                                                 "slice_duration", "slice_dim",   "phase_dim",
                                                                 "freq_dim",       "dim",         "pixdim",
...
main(int argc, const char * argv[])
{
  ...
  {
    if (rv < 0)
      FREE_RETURN(1); /* free opts memory, and return */
    else
      FREE_RETURN(0); /* valid usage */
  }
  ...
  if (opts.cbl)
    FREE_RETURN(act_cbl(&opts));
  if (opts.cci)
    FREE_RETURN(act_cci(&opts));
  if (opts.copy_image)
    FREE_RETURN(act_copy(&opts));
  if (opts.dts || opts.dci)
    FREE_RETURN(act_disp_ci(&opts));

  /* perform modifications early, in case we allow multiple actions */
  if (opts.strip && ((rv = act_strip(&opts)) != 0))
    FREE_RETURN(rv);

  if (opts.add_exts && ((rv = act_add_exts(&opts)) != 0))
    FREE_RETURN(rv);
  if (opts.rm_exts && ((rv = act_rm_ext(&opts)) != 0))
    FREE_RETURN(rv);

  if (opts.mod_hdr && ((rv = act_mod_hdrs(&opts)) != 0))
    FREE_RETURN(rv);
  if (opts.mod_hdr2 && ((rv = act_mod_hdr2s(&opts)) != 0))
    FREE_RETURN(rv);
  if (opts.mod_nim && ((rv = act_mod_nims(&opts)) != 0))
    FREE_RETURN(rv);

  if ((opts.swap_hdr || opts.swap_ana || opts.swap_old) && ((rv = act_swap_hdrs(&opts)) != 0))
    FREE_RETURN(rv);

  /* if a diff, return whether, a difference exists (like the UNIX command) */
  if (opts.diff_hdr && ((rv = act_diff_hdrs(&opts)) != 0))
    FREE_RETURN(rv);
  if (opts.diff_hdr1 && ((rv = act_diff_hdr1s(&opts)) != 0))
    FREE_RETURN(rv);
  if (opts.diff_hdr2 && ((rv = act_diff_hdr2s(&opts)) != 0))
    FREE_RETURN(rv);
  if (opts.diff_nim && ((rv = act_diff_nims(&opts)) != 0))
    FREE_RETURN(rv);
  ...
  if (opts.disp_exts && ((rv = act_disp_exts(&opts)) != 0))
    FREE_RETURN(rv);
  if (opts.disp_cext && ((rv = act_disp_cext(&opts)) != 0))
    FREE_RETURN(rv);
  if (opts.disp_hdr && ((rv = act_disp_hdr(&opts)) != 0))
    FREE_RETURN(rv);
  if (opts.disp_hdr1 && ((rv = act_disp_hdr1(&opts)) != 0))
    FREE_RETURN(rv);
  if (opts.disp_hdr2 && ((rv = act_disp_hdr2(&opts)) != 0))
    FREE_RETURN(rv);
  if (opts.disp_nim && ((rv = act_disp_nims(&opts)) != 0))
    FREE_RETURN(rv);
  if (opts.disp_ana && ((rv = act_disp_anas(&opts)) != 0))
    FREE_RETURN(rv);
 ...
 *----------------------------------------------------------------------*/
int
process_opts(int argc, const char * argv[], nt_opts * opts)
{
  int ac, count;
  ...
  for (ac = 1; ac < argc; ac++)
  {
    ...
    else if (!strcmp(argv[ac], "-add_ext"))
    {
      ...
      if (add_int(&opts->etypes, (int)new_ecode))
        return -1;
      ac++;
      CHECK_NEXT_OPT(ac, argc, "-add_ext");
      if (add_string(&opts->elist, argv[ac]))
        return -1; /* add extension */
      opts->add_exts = 1;
    }
    else if (!strcmp(argv[ac], "-add_afni_ext"))
    {
      ...
      if (add_string(&opts->elist, argv[ac]))
        return -1; /* add extension */
      if (add_int(&opts->etypes, NIFTI_ECODE_AFNI))
        return -1;
      opts->add_exts = 1;
    }
    else if (!strncmp(argv[ac], "-add_comment_ext", 12))
    {
      ...
      if (add_string(&opts->elist, argv[ac]))
        return -1; /* add extension */
      if (add_int(&opts->etypes, NIFTI_ECODE_COMMENT))
        return -1;
      opts->add_exts = 1;
    }
    ...
    else if (!strcmp(argv[ac], "-copy_brick_list") || !strcmp(argv[ac], "-copy_im") || !strcmp(argv[ac], "-cbl"))
    {
      if (!strcmp(argv[ac], "-copy_im"))
        fprintf(stderr, "** -copy_im is being deprecated, please use -cbl\n");
      opts->cbl = 1;
    }
    else if (!strcmp(argv[ac], "-copy_collapsed_image") || !strcmp(argv[ac], "-cci"))
    {
    ...
    else if (!strcmp(argv[ac], "-disp_collapsed_image") || !strcmp(argv[ac], "-disp_ci"))
    {
      /* we need to read in the 7 dimension values */
      int index;
      opts->ci_dims[0] = 0;
      ...
      {
        ac++;
        CHECK_NEXT_OPT_MSG(ac, argc, "-disp_ci", "7 dimension values are required");
        if (!isdigit(argv[ac][0]) && strcmp(argv[ac], "-1") != 0)
        {
          fprintf(stderr,
    ...
    else if (!strcmp(argv[ac], "-disp_ts") || !strcmp(argv[ac], "-dts"))
    {
    ...
    else if (!strcmp(argv[ac], "-field"))
    {
      ...
      else
      {
        ...
        if (add_string(&opts->flist, argv[ac]))
          return -1; /* add field */
      }
    }
    else if (!strncmp(argv[ac], "-infiles", 3))
    {
      ...
      for (count = 0; (ac < argc) && (argv[ac][0] != '-'); ac++, count++)
        if (add_string(&opts->infiles, argv[ac]))
          return -1; /* add field */
      if (count > 0 && ac < argc)
        ac--; /* more options to process */
      if (g_debug > 2)
        fprintf(stderr, "+d have %d file names\n", count);
    }
    ...
    else if (!strcmp(argv[ac], "-mod_field"))
    {
      ...
      if (add_string(&opts->vlist, argv[ac]))
        return -1; /* add value */
    }
    ...
    else if (!strcmp(argv[ac], "-rm_ext"))
    {
      ...
      else
      {
        ...
        {
          fprintf(stderr, "** '-rm_ext' requires an extension index (read '%s')\n", argv[ac]);
          return -1;
        }
        if (add_string(&opts->elist, argv[ac]))
          return -1;
  ...
  if (opts->make_im)
  {
    ...
    opts->cbl = 1;
    if (add_string(&opts->infiles, NT_MAKE_IM_NAME))
      return -1;
  }
 ...
 *----------------------------------------------------------------------*/
int
verify_opts(nt_opts * opts, const char * prog)
{
  ...
  ac += (opts->diff_hdr || opts->diff_hdr1 || opts->diff_hdr2 || opts->diff_nim) ? 1 : 0;
  ac += (opts->disp_hdr || opts->disp_hdr1 || opts->disp_hdr2 || opts->disp_nim || opts->disp_ana || opts->disp_exts ||
         opts->disp_cext)
          ? 1
          : 0;
  ac += opts->mod_hdr;
  ...
  else if ((opts->elist.len > 0 || opts->mod_hdr || opts->mod_hdr2 || opts->mod_nim || opts->swap_hdr ||
            opts->swap_ana || opts->swap_old) &&
           !opts->overwrite)
  {
    if (opts->infiles.len > 1)
    {
      fprintf(stderr,
              "** without -overwrite, only one input file may be"
              " modified at a time\n");
      errs++;
    }
    else if (!opts->prefix)
  ...
  if (g_debug > 1)
    fprintf(stderr, "+d options seem valid\n");
 ...
 *----------------------------------------------------------------------*/
int
fill_cmd_string(nt_opts * opts, int argc, const char * argv[])
{
  char * cp;
  ...
  for (int ac = 1; ac < argc; ac++)
  {
    if (skip)
    {
      skip--;
      continue;
    } /* then skip these arguments */

    size_t tmp_len = strlen(argv[ac]);
    if (tmp_len + 3 >= remain)
    { /* extra 3 for space and possible '' */
      fprintf(stderr, "FCS: no space remaining for command, continuing...\n");
      return 1;
    }
 ...
 *----------------------------------------------------------------------*/
int
add_int(int_list * ilist, int val)
{
  if (ilist->len == 0)
 ...
 *----------------------------------------------------------------------*/
int
add_string(str_list * slist, const char * str)
{
  if (slist->len == 0)
 ...
 *----------------------------------------------------------------------*/
int
usage(const char * prog, int level)
{
  int c, len;
  if (level == USE_SHORT)
  {
...
use_full(void)
{
  printf("nifti_tool - display, modify or compare nifti headers\n"
         "\n"
 ...
 *----------------------------------------------------------------------*/
int
disp_nt_opts(const char * mesg, nt_opts * opts)
{
  int c;
...
act_add_exts(nt_opts * opts)
{
  ...
  {
    fprintf(stderr,
            "+d adding %d extensions to %d files...\n"
            "   extension types are: ",
            opts->elist.len,
            opts->infiles.len);
  ...
  for (fc = 0; fc < opts->infiles.len; fc++)
  {
    ...
    if (!nim)
      return 1; /* errors come from the library */
    ...
    if (opts->keep_hist && nifti_add_extension(nim, opts->command, (int)strlen(opts->command), NIFTI_ECODE_COMMENT))
      fprintf(stderr, "** failed to add command to image as extension\n");

    if (opts->prefix && nifti_set_filenames(nim, opts->prefix, !opts->overwrite, 1))
    {
      nifti_image_free(nim);
      return 1;
    }
...
act_rm_ext(nt_opts * opts)
{
  ...
  else if (opts->overwrite && opts->infiles.len != 1 && strcmp(opts->elist.list[0], "-1") != 0)
  {
  ...
  if (g_debug > 1)
    fprintf(stderr, "+d removing extension index %d\n", ext_ind);
  ...
  for (fc = 0; fc < opts->infiles.len; fc++)
  {
    ...
    if (g_debug > 1)
      fprintf(
        stderr, "+d writing %s with %d fewer extension(s)\n", nim->fname, ext_ind == -1 ? num_ext : opts->elist.len);
 ...
 *----------------------------------------------------------------------*/
int
remove_ext_list(nifti_image * nim, const char ** elist, int len)
{
  int * marks;
  int   c, ec, extval;
  ...
  {
    if (!marks[ec])
      continue; /* do not delete this one */
  ...
  if (g_debug > 3)
    fprintf(stderr, "-d done removing extensions\n");

  if (nim->num_ext == 0)
  { /* did we trash the only extension? */
    if (g_debug > 1)
...
act_diff_hdrs(nt_opts * opts)
{
  ...
  nhdr0 = nt_read_header(opts->infiles.list[0], &nva, NULL, 0, opts->new_datatype, opts->new_dim);
  if (!nhdr0)
    return 1; /* errors have been printed */

  nhdr1 = nt_read_header(opts->infiles.list[1], &nvb, NULL, 0, opts->new_datatype, opts->new_dim);
  if (!nhdr1)
  {
    free(nhdr0);
    return 1;
  ...
  {
    fprintf(stderr,
            "** %s is NIFTI-%d, while %s is NIFTI-%d\n"
...
act_diff_hdr1s(nt_opts * opts)
{
  ...
  /* nhdr0 = nt_read_header(opts, opts->infiles.list[0], NULL, 0); */
  nhdr0 = nt_read_header(opts->infiles.list[0], &nv, NULL, 0, opts->new_datatype, opts->new_dim);
  if (!nhdr0)
    return 1; /* errors have been printed */
  if (!nifti_hdr1_looks_good(nhdr0))
    fprintf(stderr, "** warning: invalid NIFTI-1 hdr: %s\n", opts->infiles.list[0]);

  nhdr1 = nt_read_header(opts->infiles.list[1], &nv, NULL, 0, opts->new_datatype, opts->new_dim);
  if (!nhdr1)
  {
    free(nhdr0);
    return 1;
...
act_check_hdrs(nt_opts * opts)
{
  ...
  for (filenum = 0; filenum < opts->infiles.len; filenum++)
  {
    ...
    nhdr = nt_read_header(opts->infiles.list[filenum], &nver, NULL, 0, opts->new_datatype, opts->new_dim);
    if (!nhdr)
      continue; /* errors are printed from library */
...
act_disp_exts(nt_opts * opts)
{
  ...
  for (fc = 0; fc < opts->infiles.len; fc++)
  {
    ...
    if (!nim)
      return 1; /* errors are printed from library */
...
act_disp_cext(nt_opts * opts)
{
  ...
  for (fc = 0; fc < opts->infiles.len; fc++)
  {
    ...
    for (ec = 0; ec < nim->num_ext; ec++)
    {
      if (nim->ext_list[ec].ecode != NIFTI_ECODE_CIFTI)
        continue;
      found++;
...
act_disp_hdr(nt_opts * opts)
{
  ...
  for (filenum = 0; filenum < opts->infiles.len; filenum++)
  {
    /* do not validate the header structure */
    nver = 0;
    nhdr = nt_read_header(opts->infiles.list[filenum], &nver, NULL, g_debug > 1, opts->new_datatype, opts->new_dim);
    if (!nhdr)
      return 1; /* errors are printed from library */
    ...
    nfields = opts->flist.len > 0 ? opts->flist.len : nver <= 1 ? NT_HDR1_NUM_FIELDS : NT_HDR2_NUM_FIELDS;
...
act_disp_anas(nt_opts * opts)
{
  ...
  for (filenum = 0; filenum < opts->infiles.len; filenum++)
  {
    ...
    if (g_debug > 1)
      fprintf(
        stderr, "-d analyze header is: %s\n", nifti_hdr1_looks_good((nifti_1_header *)nhdr) ? "valid" : "invalid");
...
act_mod_hdrs(nt_opts * opts)
{
  ...
  if (opts->flist.len <= 0 || opts->infiles.len <= 0)
    return 0;
  ...
  for (filec = 0; filec < opts->infiles.len; filec++)
  {
    ...
    /* do not validate the header structure */
    nhdr = nt_read_header(fname, &nver, &swap, 0, opts->new_datatype, opts->new_dim);
    if (!nhdr)
      return 1;
    ...
    if (!nifti_hdr1_looks_good(nhdr))
    {
      ...
      if (nifti_hdr2_looks_good(n2hdr))
      {
        if (g_debug > 0)
          fprintf(stderr,
                  "** refusing to modify NIFTI-2 header "
                  "as NIFTI-1 in %s\n",
                  fname);
        free(nhdr);
        free(n2hdr);
  ...
  for (filec = 0; filec < opts->infiles.len; filec++)
  {
    ...
    if (!nifti_hdr2_looks_good(nhdr))
    {
      ...
      if (nifti_hdr1_looks_good(n1hdr))
      {
        if (g_debug > 0)
          fprintf(stderr,
                  "** refusing to modify NIFTI-1 header "
                  "as NIFTI-2 in %s\n",
                  fname);
        free(nhdr);
        free(n1hdr);
  ...
  for (filec = 0; filec < opts->infiles.len; filec++)
  {
    ...
    if (!nifti_hdr1_looks_good(nhdr))
    {
      ...
      if (nifti_hdr2_looks_good(n2hdr))
      {
        ...
        if (g_debug > 0)
          fprintf(stderr,
                  "** refusing to swap NIFTI-2 header "
                  "as NIFTI-1 in %s\n",
                  fname);
        free(nhdr);
        free(n2hdr);
    ...
    if (!swap)
    { /* if not yet swapped, do as the user requested */

      if (opts->swap_old)
        old_swap_nifti_header(nhdr, NIFTI_VERSION(*nhdr));
      else
        swap_nifti_header(nhdr, opts->swap_ana ? 0 : 1);
    }
    else
    { /* swapped already: if not correct, need to undo */
 ...
 *----------------------------------------------------------------------*/
int
write_hdr_to_file(nifti_1_header * nhdr, const char * fname)
{
  znzFile fp;
  size_t  bytes;
 ...
 *----------------------------------------------------------------------*/
int
write_hdr2_to_file(nifti_2_header * nhdr, const char * fname)
{
  znzFile fp;
  size_t  bytes;
 ...
 *----------------------------------------------------------------------*/
int
modify_all_fields(void * basep, nt_opts * opts, field_s * fields, int flen)
{
  field_s * fp;
  ...
  for (lc = 0; lc < opts->flist.len; lc++)
  {
    ...
    for (fc = 0; fc < flen; fc++, fp++)
      if (strcmp(opts->flist.list[lc], fp->name) == 0)
        break;
 ...
 *----------------------------------------------------------------------*/
int
modify_field(void * basep, field_s * field, const char * data)
{
  float        fval;
  ...
  switch (field->type)
  {
    ...
    case DT_INT8:
    {
      ...
      for (fc = 0; fc < field->len; fc++)
      {
        ...
        {
          fprintf(stderr, "** mod val #%d (= %d) outside byte range [-%d,%d]\n", fc, val, max + 1, max);
          return 1;
        }
 ...
 *----------------------------------------------------------------------*/
static int
convert_datatype(nifti_image * nim, nifti_brick_list * NBL, int new_type, int verify, int fail_choice)
{
  void * newdata = NULL;
  int    rv;
  ...
  {
    fprintf(stderr,
            "++ convert datatype: %s to %s, v,f = %d,%d\n",
  ...
  rv = convert_raw_data(&newdata, nim->data, nim->datatype, new_type, nim->nvox, verify);
  ...
  if (rv > 0)
    fprintf(stderr,
            "** %s: inaccurate data conversion from %s to %s\n",
  ...
  {
    if (g_debug > 2)
      fprintf(stderr, "-- convert_RD: destroying new data\n");
    free(newdata);
    newdata = NULL;
  ...
  if (g_debug > 2)
    fprintf(stderr, "-- convert_RD: keeping new data\n");
 ...
 *----------------------------------------------------------------------*/
static int
convert_NBL_data(nifti_brick_list * NBL, int old_type, int new_type, int verify, int fail_choice)
{
  ...
  if (g_debug > 1)
    fprintf(stderr, "-- convert_NBL_data ...\n");
  ...
  {
    rv = convert_raw_data(NBLnew.bricks + bind, NBL->bricks[bind], old_type, new_type, nbvals, verify);
  ...
  if (cfail)
    fprintf(stderr,
            "** NBL %s: inaccurate data conversion from %s to %s\n",
            (fail_choice == 2) ? "error" : "warning",
  ...
  {
    if (g_debug > 2)
      fprintf(stderr, "-- cNBLd: destroying new data\n");
    nifti_free_NBL(&NBLnew);
    return 1;
  }

  if (g_debug > 2)
    fprintf(stderr, "-- NBL convert_RD: keeping new data\n");
 ...
 *----------------------------------------------------------------------*/
static int
convert_raw_data(void ** retdata, void * olddata, int old_type, int new_type, int64_t nvox, int verify)
{
  void *       newdata = NULL;
  const char * typestr;
  ...
  *retdata = NULL; /* init, just in case */
  ...
  switch (new_type)
  {
    ...
    case NIFTI_TYPE_INT8: /* new type */
    {
      switch (old_type)
      {
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, char, olddata, char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, char, olddata, char, nvox);
          tried = 1;
          break;
        }
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, char, olddata, unsigned char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, char, olddata, unsigned char, nvox);
          tried = 1;
          break;
        }
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, char, olddata, short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, char, olddata, short, nvox);
          tried = 1;
          break;
        }
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, char, olddata, unsigned short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, char, olddata, unsigned short, nvox);
          tried = 1;
          break;
        }
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, char, olddata, int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, char, olddata, int, nvox);
          tried = 1;
          break;
        }
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, char, olddata, unsigned int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, char, olddata, unsigned int, nvox);
          tried = 1;
          break;
        }
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, char, olddata, int64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, char, olddata, int64_t, nvox);
          tried = 1;
          break;
        }
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, char, olddata, uint64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, char, olddata, uint64_t, nvox);
          tried = 1;
          break;
        }
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, char, olddata, float, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, char, olddata, float, nvox);
          tried = 1;
          break;
        }
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, char, olddata, double, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, char, olddata, double, nvox);
          tried = 1;
          break;
        }
    ...
    case NIFTI_TYPE_UINT8: /* new type */
    {
      switch (old_type)
      {
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned char, olddata, char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned char, olddata, char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT8:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned char, olddata, unsigned char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned char, olddata, unsigned char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned char, olddata, short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned char, olddata, short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned char, olddata, unsigned short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned char, olddata, unsigned short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned char, olddata, int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned char, olddata, int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned char, olddata, unsigned int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned char, olddata, unsigned int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned char, olddata, int64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned char, olddata, int64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned char, olddata, uint64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned char, olddata, uint64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned char, olddata, float, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned char, olddata, float, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned char, olddata, double, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned char, olddata, double, nvox);
          tried = 1;
          break;
        }
    ...
    case NIFTI_TYPE_INT16: /* new type */
    {
      switch (old_type)
      {
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, short, olddata, char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, short, olddata, char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT8:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, short, olddata, unsigned char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, short, olddata, unsigned char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, short, olddata, short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, short, olddata, short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, short, olddata, unsigned short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, short, olddata, unsigned short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, short, olddata, int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, short, olddata, int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, short, olddata, unsigned int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, short, olddata, unsigned int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, short, olddata, int64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, short, olddata, int64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, short, olddata, uint64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, short, olddata, uint64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, short, olddata, float, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, short, olddata, float, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, short, olddata, double, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, short, olddata, double, nvox);
          tried = 1;
          break;
        }
    ...
    case NIFTI_TYPE_UINT16: /* new type */
    {
      switch (old_type)
      {
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned short, olddata, char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned short, olddata, char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT8:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned short, olddata, unsigned char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned short, olddata, unsigned char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned short, olddata, short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned short, olddata, short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned short, olddata, unsigned short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned short, olddata, unsigned short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned short, olddata, int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned short, olddata, int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned short, olddata, unsigned int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned short, olddata, unsigned int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned short, olddata, int64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned short, olddata, int64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned short, olddata, uint64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned short, olddata, uint64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned short, olddata, float, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned short, olddata, float, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned short, olddata, double, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned short, olddata, double, nvox);
          tried = 1;
          break;
        }
    ...
    case NIFTI_TYPE_INT32: /* new type */
    {
      switch (old_type)
      {
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int, olddata, char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int, olddata, char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT8:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int, olddata, unsigned char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int, olddata, unsigned char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int, olddata, short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int, olddata, short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int, olddata, unsigned short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int, olddata, unsigned short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int, olddata, int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int, olddata, int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int, olddata, unsigned int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int, olddata, unsigned int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int, olddata, int64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int, olddata, int64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int, olddata, uint64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int, olddata, uint64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int, olddata, float, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int, olddata, float, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int, olddata, double, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int, olddata, double, nvox);
          tried = 1;
          break;
        }
    ...
    case NIFTI_TYPE_UINT32: /* new type */
    {
      switch (old_type)
      {
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned int, olddata, char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned int, olddata, char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT8:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned int, olddata, unsigned char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned int, olddata, unsigned char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned int, olddata, short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned int, olddata, short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned int, olddata, unsigned short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned int, olddata, unsigned short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned int, olddata, int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned int, olddata, int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned int, olddata, unsigned int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned int, olddata, unsigned int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned int, olddata, int64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned int, olddata, int64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned int, olddata, uint64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned int, olddata, uint64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned int, olddata, float, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned int, olddata, float, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, unsigned int, olddata, double, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, unsigned int, olddata, double, nvox);
          tried = 1;
          break;
        }
    ...
    case NIFTI_TYPE_INT64: /* new type */
    {
      switch (old_type)
      {
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int64_t, olddata, char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int64_t, olddata, char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT8:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int64_t, olddata, unsigned char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int64_t, olddata, unsigned char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int64_t, olddata, short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int64_t, olddata, short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int64_t, olddata, unsigned short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int64_t, olddata, unsigned short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int64_t, olddata, int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int64_t, olddata, int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int64_t, olddata, unsigned int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int64_t, olddata, unsigned int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int64_t, olddata, int64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int64_t, olddata, int64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int64_t, olddata, uint64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int64_t, olddata, uint64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int64_t, olddata, float, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int64_t, olddata, float, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, int64_t, olddata, double, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, int64_t, olddata, double, nvox);
          tried = 1;
          break;
        }
    ...
    case NIFTI_TYPE_UINT64: /* new type */
    {
      switch (old_type)
      {
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, uint64_t, olddata, char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, uint64_t, olddata, char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT8:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, uint64_t, olddata, unsigned char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, uint64_t, olddata, unsigned char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, uint64_t, olddata, short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, uint64_t, olddata, short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, uint64_t, olddata, unsigned short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, uint64_t, olddata, unsigned short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, uint64_t, olddata, int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, uint64_t, olddata, int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, uint64_t, olddata, unsigned int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, uint64_t, olddata, unsigned int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, uint64_t, olddata, int64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, uint64_t, olddata, int64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, uint64_t, olddata, uint64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, uint64_t, olddata, uint64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, uint64_t, olddata, float, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, uint64_t, olddata, float, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, uint64_t, olddata, double, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, uint64_t, olddata, double, nvox);
          tried = 1;
          break;
        }
    ...
    case NIFTI_TYPE_FLOAT32: /* new type */
    {
      switch (old_type)
      {
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, float, olddata, char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, float, olddata, char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT8:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, float, olddata, unsigned char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, float, olddata, unsigned char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, float, olddata, short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, float, olddata, short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, float, olddata, unsigned short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, float, olddata, unsigned short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, float, olddata, int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, float, olddata, int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, float, olddata, unsigned int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, float, olddata, unsigned int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, float, olddata, int64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, float, olddata, int64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, float, olddata, uint64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, float, olddata, uint64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, float, olddata, float, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, float, olddata, float, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, float, olddata, double, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, float, olddata, double, nvox);
          tried = 1;
          break;
        }
    ...
    case NIFTI_TYPE_FLOAT64: /* new type */
    {
      switch (old_type)
      {
        ...
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, double, olddata, char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, double, olddata, char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT8:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, double, olddata, unsigned char, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, double, olddata, unsigned char, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, double, olddata, short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, double, olddata, short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT16:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, double, olddata, unsigned short, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, double, olddata, unsigned short, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, double, olddata, int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, double, olddata, int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, double, olddata, unsigned int, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, double, olddata, unsigned int, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_INT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, double, olddata, int64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, double, olddata, int64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_UINT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, double, olddata, uint64_t, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, double, olddata, uint64_t, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT32:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, double, olddata, float, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, double, olddata, float, nvox);
          tried = 1;
          break;
        }
        case NIFTI_TYPE_FLOAT64:
        { /* old type */
          if (verify)
            NT_DCONVERT_W_CHECKS(newdata, double, olddata, double, nvox, errs);
          else
            NT_DCONVERT_NO_CHECKS(newdata, double, olddata, double, nvox);
          tried = 1;
          break;
        }
  ...
  {
    fprintf(stderr,
            "** CND: did not try to convert %s to %s\n",
            nifti_datatype_to_string(old_type),
            nifti_datatype_to_string(new_type));
    free(newdata);
    return -1;
 ...
 *----------------------------------------------------------------------*/
static int
is_valid_conversion_type(int dtype)
{
  switch (dtype)
  {
    case NIFTI_TYPE_INT8:
 ...
 *----------------------------------------------------------------------*/
static int
is_lossless(int old_type, int new_type)
{
  ...
  if (!nifti_datatype_is_valid(old_type, 1) || !nifti_datatype_is_valid(new_type, 1))
  {
    if (g_debug > 0)
...
int_size_of_type(int dtype)
{
  switch (dtype)
  {
    ...
    case DT_BINARY:
      return 1; /* these use the full size */
    case NIFTI_TYPE_UINT8:
      return 8;
    case NIFTI_TYPE_UINT16:
      return 16;
    ...
    case NIFTI_TYPE_INT8:
      return 7; /* these lose 1 sign bit */
    case NIFTI_TYPE_INT16:
      return 15;
    case NIFTI_TYPE_INT32:
      return 31;
    ...
    case NIFTI_TYPE_RGB24:
      return 24; /* RGB: full size as UINT */
    case NIFTI_TYPE_RGBA32:
      return 32;

    /* have complex match the float of half size */
    case NIFTI_TYPE_FLOAT32:
      return 24; /* float: use mantissa */
    case NIFTI_TYPE_COMPLEX64:
      return 24; /* complex: match half float */
    case NIFTI_TYPE_FLOAT64:
      return 53;
 ...
 *----------------------------------------------------------------------*/
int
fill_hdr1_field_array(field_s * nh_fields)
{
  ...
  if (check_total_size("nifti_1_header test: ", nh_fields, NT_HDR1_NUM_FIELDS, sizeof(nifti_1_header)))
    return 1;
 ...
 *----------------------------------------------------------------------*/
int
fill_hdr2_field_array(field_s * nh_fields)
{
  field_s * nhf = nh_fields;
  int       rv, errs;
  ...
  /* failure here is a serious problem */
  if (check_total_size("nifti_2_header test: ", nh_fields, NT_HDR2_NUM_FIELDS, sizeof(nifti_2_header)))
    return 1;
 ...
 *----------------------------------------------------------------------*/
int
fill_nim1_field_array(field_s * nim_fields)
{
  ...
  if (g_debug > 3) /* failure here is not an error condition */
    check_total_size("nifti1_image test: ", nim_fields, NT_NIM_NUM_FIELDS, sizeof(nifti1_image));
 ...
 *----------------------------------------------------------------------*/
int
fill_nim2_field_array(field_s * nim_fields)
{
  field_s * nif = nim_fields;
  int       rv, errs;
  ...
  if (g_debug > 4) /* failure here is not an error condition */
    check_total_size("nifti2_image test: ", nim_fields, NT_NIM_NUM_FIELDS, sizeof(nifti2_image));
 ...
 *----------------------------------------------------------------------*/
int
fill_ana_field_array(field_s * ah_fields)
{
  ...
  /* failure here is a serious problem */
  if (check_total_size("nifti_analyze75 test: ", ah_fields, NT_ANA_NUM_FIELDS, sizeof(nifti_analyze75)))
    return 1;
 ...
 *----------------------------------------------------------------------*/
int
check_total_size(const char * mesg, field_s * fields, int nfields, int tot_size)
{
  field_s * fp;
  int       c, total;
  int       bad_offs;
 ...
 *----------------------------------------------------------------------*/
int
fill_field(field_s * fp, int type, int offset, int num, const char * name)
{
  fp->type = type;
  fp->offset = offset;
...
field_type_str(int type)
{
  ...
  if (type == DT_FLOAT64)
    return "DT_FLOAT64";
  if (type == DT_COMPLEX64)
    return "DT_COMPLEX64";
  if (type == DT_COMPLEX128)
    return "DT_COMPLEX128";

  if (type == DT_RGB24)
    return "DT_RGB24";

  if (type == NT_DT_STRING)
    return "NT_DT_STRING";
  if (type == NT_DT_POINTER)
    return "NT_DT_POINTER";
  if (type == NT_DT_CHAR_PTR)
    return "NT_DT_CHAR_PTR"; /* longest: 14 */
  if (type == NT_DT_EXT_PTR)
 ...
 *----------------------------------------------------------------------*/
int
disp_field_s_list(const char * mesg, field_s * fp, int nfields)
{
  int c, total = 0;
 ...
 *----------------------------------------------------------------------*/
int
disp_field(const char * mesg, field_s * fieldp, void * str, int nfields, int header)
{
  field_s * fp;
  int       c;
  ...
  for (c = 0; c < nfields; c++, fp++)
  {
    ...
    if (g_debug > 0)
      fprintf(
        stdout, "  %-*.*s %4d    %3d    ", NT_FIELD_NAME_LEN - 1, NT_FIELD_NAME_LEN - 1, fp->name, fp->offset, fp->len);
    ...
    switch (fp->type)
    {
      ...
      case NT_DT_EXT_PTR:
      {
        ...
        if (extp)
          disp_nifti1_extension(NULL, extp, 6);
        else
          fprintf(stdout, "(NULL)\n");
        break;
      }
 ...
 *----------------------------------------------------------------------*/
int
diff_field(field_s * fieldp, void * str0, void * str1, int nfields)
{
  field_s * fp;
  char *    cp0, *cp1;
  int       fnum, c, size;
  ...
  for (fnum = 0; fnum < nfields; fnum++, fp++)
  {
    switch (fp->type)
    {
      ...
      case NT_DT_EXT_PTR:
      {
        ...
        if (!(ext0 && ext1))
          return 1; /* pointer diff is diff */

        /* just check size and type for a single extension */
        if (ext0->esize != ext1->esize)
          return 1;
        if (ext0->ecode != ext1->ecode)
          return 1;
 ...
 *----------------------------------------------------------------------*/
int
disp_cifti_extension(const char * mesg, const nifti1_extension * ext, int maxlen)
{
  FILE * outfp = stdout;
  int    len;
  if (mesg)
 ...
 *----------------------------------------------------------------------*/
int
disp_nifti1_extension(const char * mesg, const nifti1_extension * ext, int maxlen)
{
  FILE * outfp = stdout;
  int    len;
  if (mesg)
  ...
  if (!ext->edata)
    fprintf(outfp, "(NULL)\n");
  else if (ext->ecode == NIFTI_ECODE_AFNI || ext->ecode == NIFTI_ECODE_COMMENT || ext->ecode == NIFTI_ECODE_CIFTI)
  {
    len = ext->esize - 8;
    if (maxlen >= 0 && len > maxlen)
      len = maxlen;
 ...
 *----------------------------------------------------------------------*/
field_s *
get_hdr1_field(const char * fname, int show_fail)
{
  field_s * fp;
  int       c;
 ...
 *----------------------------------------------------------------------*/
field_s *
get_hdr2_field(const char * fname, int show_fail)
{
  field_s * fp;
  int       c;
 ...
 *----------------------------------------------------------------------*/
field_s *
get_nim_field(const char * fname, int show_fail)
{
  field_s * fp;
  int       c;
 ...
 *----------------------------------------------------------------------*/
int
diff_hdr1s(nifti_1_header * s0, nifti_1_header * s1, int display)
{
 ...
 *----------------------------------------------------------------------*/
int
diff_hdr2s(nifti_2_header * s0, nifti_2_header * s1, int display)
{
 ...
 *----------------------------------------------------------------------*/
int
diff_nims(nifti_image * s0, nifti_image * s1, int display)
{
 ...
 *----------------------------------------------------------------------*/
int
diff_hdr1s_list(nifti_1_header * s0, nifti_1_header * s1, str_list * slist, int display)
{
  field_s *     fp;
  const char ** sptr;
  int           c, ndiff = 0;
 ...
 *----------------------------------------------------------------------*/
int
diff_hdr2s_list(nifti_2_header * s0, nifti_2_header * s1, str_list * slist, int display)
{
  field_s *     fp;
  const char ** sptr;
  int           c, ndiff = 0;
 ...
 *----------------------------------------------------------------------*/
int
diff_nims_list(nifti_image * s0, nifti_image * s1, str_list * slist, int display)
{
  field_s *     fp;
  const char ** sptr;
  int           c, ndiff = 0;
...
act_disp_ci(nt_opts * opts)
{
  ...
  {
    fprintf(stderr,
            "-d displaying time series at (i,j,k) = ("
  ...
  {
    fprintf(stderr,
            "-d displaying collapsed image for %d datasets...\n\n"
            "   dims = ",
            opts->infiles.len);
    disp_raw_data(opts->ci_dims, DT_INT64, 8, ' ', 1);
  }
  ...
  for (filenum = 0; filenum < opts->infiles.len; filenum++)
  {
    ...
    if (g_debug > 0)
    {
      fprintf(stdout, "\ndataset '%s' @ (", nim->fname);
      if (opts->dts)
        disp_raw_data(opts->ci_dims + 1, DT_INT64, 3, ' ', 0);
      else
        disp_raw_data(opts->ci_dims + 1, DT_INT64, 7, ' ', 0);
      fprintf(stdout, ")\n");
    }
...
int
disp_raw_data(void * data, int type, int nvals, char space, int newline)
{
 ...
 *----------------------------------------------------------------------*/
int
act_run_misc_tests(nt_opts * opts)
{
  nifti_image * nim;
  const char *  fname;
  int           fc;
 ...
 *----------------------------------------------------------------------*/
int
nt_run_misc_nim_tests(nifti_image * nim)
{
  ...
  /* test nifti_read_subregion_image() - just get one voxel */
  {
    ...
    printf("== subregion: rv=%" PRId64 ", dptr=%p, data=", rval, (void *)dptr);
    if (dptr)
      disp_raw_data((char *)dptr, nim->datatype, 1, ' ', 1);
    free(dptr);
  }
...
static int
nt_test_dmat44_quatern(nifti_image * nim)
{
  ...
  dmat = nim->sto_xyz;
  dmat = nifti_make_orthog_dmat44(dmat.m[0][0],
                                  dmat.m[0][1],
                                  dmat.m[0][2],
                                  dmat.m[1][0],
                                  dmat.m[1][1],
  ...
  nifti_dmat44_to_quatern(nim->sto_xyz, &qb, &qc, &qd, &qx, &qy, &qz, &dx, &dy, &dz, &qfac);
  ...
  nifti_mat44_to_quatern(mat0, &fb, &fc, &fd, &fx, &fy, &fz, &x, &y, &z, &fac);
  ...
  mat = nifti_make_orthog_mat44(mat.m[0][0],
                                mat.m[0][1],
                                mat.m[0][2],
                                mat.m[1][0],
                                mat.m[1][1],
...
static int
nt_disp_mat44_orient(const char * mesg, mat44 mat)
{
  int i, j, k;
...
static double
dmat44_max_fabs(nifti_dmat44 m)
{
  double max = 0.0;
  int    i, j;
  for (i = 0; i < 4; i++)
...
act_cbl(nt_opts * opts)
{
  ...
  if (g_debug > 1)
    fprintf(stderr, "+d sub-bricks loaded\n");
...
nt_image_read(nt_opts * opts, const char * fname, int read_data, int make_ver)
{
  ...
  if (strncmp(fname, NT_MAKE_IM_NAME, strlen(NT_MAKE_IM_NAME)) != 0)
  {
    ...
    {
      if (convert_datatype(nim, NULL, opts->convert2dtype, opts->cnvt_verify, opts->cnvt_fail_choice))
      {
        nifti_image_free(nim);
        return NULL;
      }
...
nt_read_header(const char * fname, int * nver, int * swapped, int check, int new_datatype, int64_t new_dim[8])
{
  ...
  if (strncmp(fname, NT_MAKE_IM_NAME, strlen(NT_MAKE_IM_NAME)) != 0)
  {
    ...
    if (!nver || !*nver)
      return nifti_read_header(fname, nver, check);
    ...
    if (*nver == 2)
      return nifti_read_n2_hdr(fname, swapped, check);
    ...
    if (!nptr)
      return NULL;
    if (g_debug > 1)
      fprintf(stderr, "-d have NIFTI-%d header, %s\n", nv, fname);
...
nt_read_bricks(nt_opts * opts, char * fname, int len, int64_t * list, nifti_brick_list * NBL)
{
  ...
  if (strncmp(fname, NT_MAKE_IM_NAME, strlen(NT_MAKE_IM_NAME)) != 0)
  {
    ...
    {
      /* convert each brick */
      if (convert_datatype(nim, NBL, opts->convert2dtype, opts->cnvt_verify, opts->cnvt_fail_choice))
      {
        nifti_image_free(nim);
        nifti_free_NBL(NBL);
        NBL->nbricks = 0;
